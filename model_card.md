...
The biggest thing I learned from this project is how much weight choices matter. I went in thinking the hard part would be writing the scoring logic, but honestly that was pretty straightforward once I had the recipe. The hard part was realizing that changing one number — like dropping genre from 2.0 to 1.0 — completely changes which songs "win" and whether the results feel right. That's something I didn't expect at all. It made me think about how real platforms like Spotify probably have teams of engineers just tuning those weights constantly based on user feedback.

Using AI to help write the code was genuinely useful, especially for getting the structure right quickly. But I did have to double-check the edge cases myself — like making sure the CSV loading actually converted numbers to floats, and making sure the OOP class and the functional functions were behaving consistently. The AI didn't always think about those kinds of quiet bugs.

What surprised me most is that even three rules — genre, mood, energy — can produce results that actually feel intentional. When I ran the Chill Lofi profile and got Library Rain and Midnight Coding at the top, it genuinely felt like a good recommendation. It felt "smart" even though under the hood it's just addition. I think that's kind of the point — a lot of what feels like AI magic is really just well-chosen features and weights.

If I kept going with this, I'd add real user feedback (like skips and replays) to start building collaborative filtering on top of the content-based system. I'd also want to handle the diversity problem so you don't get three lofi songs in a row just because lofi dominates the catalog. That feels like the most realistic next step.
